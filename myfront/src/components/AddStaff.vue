...
export default {
  ...
  created() {
    ...
    fetch(commons.API_URL + '/room/staff-order/' + this.$route.params.idRoomOrder, getStaffList)
      .then(async response => {
  ...
  methods: {
    add_staff() {
      ...
      fetch(commons.API_URL + '/room/staff-order/' + this.$route.params.idRoomOrder, addStaff)
        .then(async response => {
          const data = await response.json()
